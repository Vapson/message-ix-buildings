...
for(s in scenarios[1:7]){
  ...
  sturm_scenarios <- run_scenario(run = s,
                                  ...
                                  geo_level_report="region_bld",#"R12", region_bld", # Level for reporting
                                  region_select = NULL, #list("region_bld", c("R32IND")), 
                                  yrs = c(seq(2020,2060,5),seq(2070,2100,10)), #c(seq(2020,2060,5),seq(2070,2100,10)),# seq(2020,2030,5),
                                  input_mode = "csv",
                                  mod_arch = "stock",
                                  mod_new = "endogenous", #"external"
                                  mod_ren = "endogenous", #"external"
                                  report_type = c("STURM"), # ,"STURM","MESSAGE","NAVIGATE" # Available reports: c("MESSAGE","STURM","IRP","NGFS","NAVIGATE")
