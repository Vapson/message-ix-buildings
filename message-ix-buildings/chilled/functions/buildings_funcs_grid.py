...
def calc_vdd_tmax_c_d(
    t_out_ave, t_max_c, t_bal_c
):  # DEGREE DAYS should be calculated day by day
    "This returns the variable cooling degree days based on Tmax"
    # vdd_tmax_c = (t_out_ave - t_max_c).where((t_out_ave >= t_max_c))
    vdd_tmax_c = (t_out_ave - t_bal_c).where((t_out_ave >= t_max_c))
    vdd_tmax_c = vdd_tmax_c.fillna(0)
    return vdd_tmax_c
...
def Q_c_tmax_d(H_tr, H_v_cl, vdd_tmax_c, f_c):  #
    "This returns the monthly cooling energy (MJ) based on variable degree days"
    return (
        (H_tr + H_v_cl)
        * f_c
...
def calc_E_c_fan_d(f_f, P_f, t_out_ave, t_bal_c, t_max_c, area_fan):  #
    "This returns the electricity requirement for fans"
    Nf = xr.where((t_out_ave > t_bal_c) & (t_out_ave < t_max_c), 1, 0)
    # Nf = (t_out_ave - t_max_c).where((t_out_ave >= t_bal_c))
    # Nf.values[Nf > 0] = 0
    # Nf.values[Nf < 0] = 1
    # Nf = Nf.fillna(0)    
    # Nf = np.where((t_out_ave > t_bal_c) & (t_out_ave < t_max_c) , 1, 0) # lead to: OOM
    return (
        f_f * P_f * Nf * 86400.0 / (np.power(10.0, 6.0) * area_fan)
    )  # Nf = number of days when (t_bal_c <= t_out_ave < t_max_c)
